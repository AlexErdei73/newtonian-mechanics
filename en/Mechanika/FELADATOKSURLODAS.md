...
$$
F_{\text{net}} = F_{s,\text{max}} = \mu_s \cdot N = 0.8 \cdot 12,753 = 10,202.4\text{ N}
$$
...
$$
|a| = \frac {F_{\text{net}}} {m} = \frac {\mu_s \cdot m \cdot g} {m} = \mu_s \cdot g = 0.8 \cdot 9.81 = 7.848\text{ }\frac {\text{m}} {\text{s}^2}
$$
...
By using traction control, the wheels operate at the limit of static friction, so the maximum accelerating force is provided by the maximum static friction ($F_{\text{net}} = F_{s,\text{max}} = \mu_s \cdot m \cdot g$):
...
A car travels on a wet asphalt road at a speed of $90\text{ }\frac{\text{km}}{\text{h}}$. On the wet surface, the coefficient of static friction between the tire and the road drops to $\mu_s = 0.5$, and the coefficient of sliding friction drops to $\mu = 0.4$. What will be the length of the stopping distance if the car brakes with a working anti-lock braking system (ABS), and what will it be if the brakes lock up (the wheels slip)?
...
In a drag race, a race car accidentally applies too much throttle at the moment of launch, causing its driven wheels to spin continuously on the ground throughout the first $4\text{ seconds}$ of acceleration. The coefficient of sliding friction between the surfaces is $\mu = 0.5$, whereas the static coefficient would have been an optimal $\mu_s = 0.9$ in the ideal case. What distance does the car cover during these $4\text{ seconds}$ of acceleration? What distance would it have left behind if it had accelerated right at the optimal static limit with the help of traction control?
