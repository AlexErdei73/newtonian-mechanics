...
$$
m \cdot g \cdot \sin \alpha \leqslant \mu_s \cdot m \cdot g \cdot \cos \alpha
$$
...
### Example
An object with a mass of $0.1\text{ kg}$ is located on an inclined plane that is $2.00\text{ m}$ high and $6.00\text{ m}$ wide. What angle does the incline enclose with the horizontal? What is the force pulling down along the incline and what is the constraint force? What is the maximum static friction force if $\mu_s = 0.5$? At what inclination angle does the object slip? The acceleration due to gravity is $g = 9.81\text{ }\frac{\text{m}}{\text{s}^2}$.
...
$$
F_{s,\text{max}} = \mu_s \cdot N = 0.5 \cdot 0.9307\text{ N} \approx 0.4654\text{ N}
$$
...
$$
\tan \alpha_{\text{max}} = \mu_s = 0.5 \implies \alpha_{\text{max}} \approx 26.57^\circ
$$
...
If the inclination angle of the plane exceeds the threshold limit of static friction, the object slips and moves downward with acceleration. The sliding friction force arising during motion depends on the coefficient of sliding friction ($F_k = \mu \cdot N$). Let's write the net force along the incline:
...
$$
F_{\text{net}} = F - F_k = m \cdot g \cdot \sin \alpha - \mu \cdot N = m \cdot g \cdot \sin \alpha - \mu \cdot m \cdot g \cdot \cos \alpha
$$
...
$$
F_k = \mu \cdot N = 0.5 \cdot 0.8162 \approx 0.4081\text{ N}
$$
...
2. During an experiment, we observe that an object starts to slip exactly when the inclination angle of the plane is raised to $20.0^\circ$. Determine the value of the coefficient of static friction ($\mu_s$) between the surfaces!
3. An object with a mass of $0.500\text{ kg}$ slides down an inclined plane with an inclination angle of $30.0^\circ$. The coefficient of sliding friction between the surfaces is $0.3$. Determine the numerical magnitude of the following forces: force along the incline ($F$), constraint force ($N$), friction force ($F_k$), and net force ($F_{\text{net}}$)!
...
5. In a warehouse, crates are slid down a smooth incline. What should be the minimum inclination angle of the plane so that the crates slip and start moving downward by themselves if the value of the coefficient of static friction is $\mu_s = 0.4$?
