...
$$
\Delta E_{\text{m}} = E_{\text{m,vég}} - E_{\text{m,kezdet}}
$$
...
A jobb oldali kifejezés pontosan azt jelenti, ami a bal oldalon van leírva, csak lényegesen rövidebb. Úgy kell értelmezni, hogy $i$ felveszi az $1, 2, 3, \dots, N$ értékeket, amit minden egyes értékre be kell helyettesíteni a szumma jel után álló kifejezéssbe (ami esetünkben csak $i$). Szóval $i = 1$-re 1, aztán $i = 2$-re 2, és így tovább. Az így kapott kifejezéseket össze kell adni. Ezáltal megkapjuk a bal oldalon álló összeget. Ez nagyon hosszú is lehet, ha pl. $N = 1000$. Ekkor 1000 tagú az összeg, ezt rövidítettük a „...” jelöléssel a bal oldalon. A szumma jel ennek még rövidebb alakja. A jobb oldalt így olvassuk: *"Szumma i egytől N-ig i"*.
...
$$
\vec{v}_{\text{TKP}} = \frac{m_1 \vec{v}_1 + m_2 \vec{v}_2}{m_1 + m_2}
$$
...
$$
\vec{v}_{\text{TKP}} = \frac{m_1 \vec{v}_1 + m_2 \vec{v}_2 + \dots + m_N \vec{v}_N}{m_1 + m_2 + \dots + m_N}
$$

Hol helyezkedik el maga a tömegközéppont? Először nézzük meg a 2 tömegpontból álló rendszert, majd általánosítsunk!

$$
x_{\text{TKP}} = \frac{m_1 x_1 + m_2 x_2}{m_1 + m_2}
$$

$$
y_{\text{TKP}} = \frac{m_1 y_1 + m_2 y_2}{m_1 + m_2}
$$

> **A tömegközéppont koordinátái a tömegpontok koordinátáinak a tömegeikkel súlyozott átlagai.**

Nézzük meg a sebesség $x$ komponensét a koordináták változásából ($v_x = \Delta x / t$):

$$
v_{\text{TKP},x} = \frac{x_{\text{TKP}} - x_{\text{TKP},0}}{t} = \frac{\frac{m_1 x_1 + m_2 x_2}{m_1 + m_2} - \frac{m_1 x_{1,0} + m_2 x_{2,0}}{m_1 + m_2}}{t}
$$

$$
v_{\text{TKP},x} = \frac{m_1 x_1 + m_2 x_2 - m_1 x_{1,0} - m_2 x_{2,0}}{(m_1 + m_2)t} = \frac{m_1 \frac{x_1 - x_{1,0}}{t} + m_2 \frac{x_2 - x_{2,0}}{t}}{m_1 + m_2}
$$

$$
v_{\text{TKP},x} = \frac{m_1 v_{1,x} + m_2 v_{2,x}}{m_1 + m_2}
$$

$v_{\text{TKP},y}$-ra teljesen hasonlóan kapjuk a megfelelő összefüggést. Ezzel igazoltuk is a $\vec{v}_{\text{TKP}}$-ra vonatkozó képletet két tömegpontból álló rendszerre. Az általánosítás teljesen hasonló, csak a képleteket szumma jellel írjuk fel $N$ tömegpontra, és úgy végezzük az algebrai műveleteket.

$$
x_{\text{TKP}} = \frac{\sum_{i = 1}^{N} m_i x_i}{M}
$$

$$
v_{\text{TKP},x} = \frac{x_{\text{TKP}} - x_{\text{TKP},0}}{t} = \frac{\frac{\sum_{i = 1}^{N} m_i x_i}{M} - \frac{\sum_{i = 1}^{N} m_i x_{i,0}}{M}}{t}
$$

$$
v_{\text{TKP},x} = \frac{\sum_{i = 1}^{N} (m_i x_i - m_i x_{i,0})}{Mt} = \frac{\sum_{i = 1}^{N} m_i \frac{x_i - x_{i,0}}{t}}{M}
$$

$$
v_{\text{TKP},x} = \frac{\sum_{i = 1}^{N} m_i v_{i,x}}{M}
$$
...
> **A tömegközéppont sebessége a tömegpontok sebességeinek a tömegeikkel súlyozott átlaga.**

### Példa

Legyen az első tömegpont tömege $0,1\text{ kg}$ és a másodiké is ugyanennyi. Mindkét pont az $x$ tengelyen helyezkedik el, kezdetben $x_{1,0} = 0$ és $x_{2,0} = 3\text{ m}$. Hol helyezkedik el a tömegközéppont? Mekkora a tömegközéppont sebessége ekkor, ha az első test sebessége $1\text{ m/s}$ és a második test nyugalomban van? Mekkora a tömegközéppont sebessége ütközés után? Mekkora a testek közös sebessége, ha az ütközés tökéletesen rugalmatlan?

$$
x_{\text{TKP},0} = \frac{m_1 x_{1,0} + m_2 x_{2,0}}{M} = \frac{0,1 \cdot 0 + 0,1 \cdot 3}{0,2} = 1,5\text{ m}
$$

A tömegközéppont a két egyenlő tömegű pont által meghatározott szakasz felezőpontja az $x$ tengelyen.

$$
v_{\text{TKP},x} = \frac{m_1 v_{1,x} + m_2 v_{2,x}}{M} = \frac{0,1 \cdot 1 + 0}{0,2} = 0,5\text{ m/s}
$$
...
2. Három test mozog egy súrlódásmentes asztalon.
    * 1. test: $m_1 = 2\text{ kg}$, $v_1 = 4\text{ m/s}$ (jobbra)
    * 2. test: $m_2 = 3\text{ kg}$, $v_2 = -2\text{ m/s}$ (balra)
    * 3. test: $m_3 = 5\text{ kg}$, $v_3 = 0$ (áll)
...
3. Egy $1000\text{ kg}$ tömegű műhold kering a Föld körül, amikor egy $50\text{ kg}$ tömegű meteorit csapódik bele. A becsapódás előtt a műhold sebessége $7000\text{ m/s}$ volt, a meteorit pedig szemből érkezett $15\,000\text{ m/s}$ sebességgel. A meteorit belefúródik a műholdba (tökéletesen rugalmatlan ütközés). Mekkora lesz a roncs sebessége közvetlenül az ütközés után?
