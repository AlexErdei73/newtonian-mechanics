...
A tömegpont impulzusmomentuma hasonlóan jön létre az impulzusból, mint a forgatónyomaték az erőből. Az origóban legyen a forgástengely, legyen ez a $z$ tengelye a koordináta-rendszernek. Ekkor az erre a tengelyre vett impulzusmomentum a következőképpen számítható ki:

$$
N_z = mvr \sin \alpha
$$

Ebben az egyenletben minden a mozgás $x$-$y$ síkban történő vetületére vonatkozik, tehát $v$ az $m$ tömegű tömegpont sebessége az $x$-$y$ síkban, $r$ pedig a tömegpont távolsága a forgástengelytől. Az $\alpha$ szög az a $180^\circ$-nál nem nagyobb szög, amelyet $r$ bezár $v$-vel az $x$-$y$ síkban. A szög előjele olyan, hogy az óramutató járásával ellentétes értelmű forgás pozitív impulzusmomentumot eredményez, az óramutató járásával egyező értelmű forgás pedig negatív impulzusmomentumot jelent.
...
$$
N_z = \sum_{i = 1}^{N} m_i v_i r_i \sin \alpha_i
$$
...
$$
N_z = \left( \sum_{i = 1}^{N} m_i r_i^2 \right) \omega = \Theta \omega
$$
...
Egy elhanyagolható tömegű merev rúdhoz rögzítve, a forgástengelyre szimmetrikusan két egyforma $0,200\text{ kg}$-os tömeg egyforma $0,300\text{ m}$ távolságra a tengelytől forog $3,00\text{ m/s}$ sebességgel. A forgás az óramutató járásával ellentétes értelmű. Számítsuk ki a rendszer impulzusmomentumát a forgástengelyre vonatkozóan!

$$
N_z = \sum_{i = 1}^{N} m_i v_i r_i = 2 \cdot 0,2 \cdot 3 \cdot 0,3 = 0,360\text{ kg}\cdot\text{m}^2\text{/s}
$$
...
$$
\Theta = \sum_{i = 1}^{N} m_i r_i^2 = 2 \cdot 0,2 \cdot 0,3^2 = 0,036\text{ kg}\cdot\text{m}^2
$$

$$
N_z = \Theta \omega = 0,036 \cdot 10 = 0,360\text{ kg}\cdot\text{m}^2\text{/s}
$$
...
$$
\Theta = 2(2m)\left(\frac{r}{2}\right)^2 = 4m \frac{r^2}{4} = m r^2 = \frac{\Theta_0}{2}
$$
...
1. Három egyforma, egyenként $0,500\text{ kg}$ tömegű pontszerű test helyezkedik el egy elhanyagolható tömegű merev rúdon. Az origóban lévő forgástengelytől mért távolságuk rendre $0,200\text{ m}$, $0,400\text{ m}$ és $0,600\text{ m}$. A rendszer az óramutató járásával ellentétes irányban forog $5,00\ 1\text{/s}$ szögsebességgel. 
    * a) Számítsd ki a rendszer tehetetlenségi nyomatékát!
    * b) Számítsd ki a rendszer impulzusmomentumát!

2. Egy $1,50\text{ kg}$ tömegű golyó egy $0,800\text{ m}$ hosszú (elhanyagolható tömegű) madzag végén körmozgást végez. A rendszer impulzusmomentuma a középpontra (forgástengelyre) vonatkozóan $12,0\text{ kg}\cdot\text{m}^2\text{/s}$ (az óramutató járásával ellentétes). 
...
4. Két azonos tömegű ($m = 2,00\text{ kg}$) pontszerű test kering egy közös tengely körül, azonos $r = 0,500\text{ m}$ sugarú pályán. Az „A” test sebessége $4,00\text{ m/s}$ az óramutató járásával ellentétes irányban, mi több a „B” test sebessége $3,00\text{ m/s}$ az óramutató járásával megegyező irányban. 
